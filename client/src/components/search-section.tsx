...
export default function SearchSection() {
  ...
  // Debounce da busca
  useEffect(() => {
    // Apenas busca se um lugar NÃO foi selecionado ainda.
    if (!selectedPlace) {
  ...
  const handleSelect = (name: string) => {
    setSearchTerm(name);
    setShowDropdown(false);
    const placeObj = results.find((place) => place.name === name);
    if (placeObj) {
      setSelectedPlace(placeObj);
      window.lojaPesquisada = {
        nome: placeObj.name,
        endereco: placeObj.formatted_address,
        ...placeObj,
      };
      window.dispatchEvent(new Event("lojaPesquisadaChange"));
      setResults([]);
      // Rola para próxima sessão
      setTimeout(() => {
        const nextSection = document.querySelector(
          "#order-form-section, #order-section, #next-section"
        );
        if (nextSection) {
  ...
  return (
    <section id="search-section" className="py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="relative flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
          <div className="w-full relative">
            <div className="relative w-full flex items-center">
              <input
                ...
                className="w-full px-6 py-4 rounded-lg bg-slenocard-gray border border-gray-600 text-white placeholder-gray-400 focus:outline-none focus:border-slenocard-orange transition-colors duration-200 pr-12"
          ...
          {/* Botão de busca removido, pesquisa é feita em tempo real ao digitar */}
        </div>
